          ...
          <table className="w-full text-xs text-left min-w-[950px]">
            <thead className="bg-slate-50 text-[11px] font-bold text-slate-600 uppercase tracking-wider border-b border-slate-200">
              <tr>
                ...
                <th className="py-2.5 px-3 whitespace-nowrap">Lote Recepción</th>
                <th className="py-2.5 px-3 whitespace-nowrap">Proveedor</th>
                <th className="py-2.5 px-3 whitespace-nowrap">Especie</th>
                <th className="py-2.5 px-3 whitespace-nowrap">Tipo Madera</th>
                <th className="py-2.5 px-3 whitespace-nowrap">Unidad</th>
                <th className="py-2.5 px-4 text-right whitespace-nowrap">Cantidad</th>
                <th className="py-2.5 px-3 whitespace-nowrap">Guía / Doc</th>
                <th className="py-2.5 px-4 whitespace-nowrap">Receptor</th>
              </tr>
            </thead>
              ...
              {data && data.length > 0 ? (
                data.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-2.5 px-4 font-mono font-bold text-right tabular-nums text-slate-900 whitespace-nowrap">
                      ...
                      {(item.yugosQuantity != null || item.reglasQuantity != null) && (
                        <div className="text-[10px] text-slate-500 font-sans font-normal">
                          {item.yugosQuantity ?? 0} Yugos · {item.reglasQuantity ?? 0} Reglas
                        </div>
                      )}
                    </td>
                    ...
                    <td className="py-2.5 px-4 text-slate-600 truncate max-w-[120px] whitespace-nowrap" title={item.receivedBy}>
                      {item.receivedBy}
                    </td>
                  </tr>
                ))
